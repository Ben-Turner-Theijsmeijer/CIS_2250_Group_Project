...
def main(argv):
  ...
  if len(argv) < 3 or len(argv) > 7:
    stopExecution("\nUsage: python question1.py <Start Month> <Start Year> <End Month> <End Year> \"Gender\" \"Filename.pdf\"\n\nRequired Fields: <Start Month> <Start Year>\nDefaults: <End Month> = 9, <End Year> = 2020, \"Gender\" = 'Both sexes', \"Filename.pdf\" = chartOut.pdf")
  ...
  unemploymentFile = openFile("datasets/unemployment.csv")
  unemploymentData = csv.reader(unemploymentFile)
  casesFile = openFile("datasets/cases_by_status_and_phu.csv")
  ...
  createChart1.createChart(StringIO(outputString), fileName)
