...
question4.py
  ...
  Question: How does age and gender affect the mortality 
          and survivability rates of individuals afflicted with COVID-19 in the different Ontario public health units?
  
  Date of Last Update: April 1, 2021.

  Functional Summary
      question4.py takes in the specified gender and PHU ID and
  ...
  Example Run of Question 4 to a csv file:
    python question4.py UNSPECIFIED 3895 > q4_output.csv
  Example Run of Create Chart 4:
...
def main(argv):
  ...
  filename = "datasets/conposcovidloc.csv"
  ...
  PHUID = argv[2]
  gender = gender.upper()
  if gender not in ["MALE","FEMALE","UNSPECIFIED","GENDER_DIVERSE"]:
    print("Usage: extract_PHU_data_by_gender.py <gender> <PHU ID>")
    print("Options for gender are 'MALE' 'FEMALE' 'UNSPECIFIED' 'GENDER_DIVERSE'")
    sys.exit(1)

  if gender == "GENDER_DIVERSE":
  ...
  age_group = [">20", "20s", "30s", "40s", "50s", "60s", "70", "80s", "90+", "Unknown"]
  ...
  print("Age_Group,Resolved_Cases,Fatal_Cases")
  for i in range (10):
    print("{}, {}, {}".format(age_group[i], resolved_array[i], fatal_array[i]))
